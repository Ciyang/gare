  ...
  slides.ex = slide({
    ...
    present : _.shuffle([
      {noun: "star", adjective: "blue", fileinit: "starBlue",
       range:[[1, 9]], targetRange: ["dhigh"]},
       {noun: "plate", adjective: "clean", fileinit: "plateClean",
        range:[[1, 9]], targetRange: ["dlow"]}
    ]),
    ...
    present_handle : function(stim) {
      ...
      // determine the position of the target object
      this.stim.target = _.sample(stim.targetRange);

      if (this.stim.target == "dhigh"){
        postarget = this.stim.poshigh;
        posnontarget = this.stim.poslow;
      }
      else {
        postarget = this.stim.poslow;
        posnontarget = this.stim.poshigh;
      }

      // if (postarget == "left"){
      //   posHint = "<sup>&lt;</sup>left";
      //   arrow = "&lt;";
      // }
      // else {
      //   posHint = "right<sup>&gt;</sup>";
      //   arrow = "&gt;";
      // }
      $(".imgpointingup"+postarget).show();
      $(".imgpointingup"+posnontarget).hide();

      $(".question").html("Is this " + stim.noun + " " + stim.adjective + "?");
