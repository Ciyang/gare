function make_slides(f) {
  ...
  slides.calibration = slide({
    ...
    button : function() {
      ...
      // adjust the width of the div that contains
      // "both" and "neither" responses
      $(".divpicwidth").css("width", 350 * this.ratio);
  ...
  slides.ex = slide({
    ...
    present_handle : function(stim) {
      ...
      $(".instruction").html("Please click on the " + stim.adjective  + " " + stim.noun + ".");

      // update the adjective in "both" and "neither" responses
      $(".adjective").html(stim.adjective);    },
...
function init() {
  ...
  // the location of the "both" response: left or right
  locboth = positions[0];

  // the location of the "neither" response: left or right
  locneither = positions[1];

  // update words and radio button values
  $(".radio" + locboth).attr("value", "both");
  $(".radio" + locneither).attr("value", "neither");

  $(".text" + locboth).html("But <span style='text-decoration: underline'>both</span> are");
  $(".text" + locneither).html("But <span style='text-decoration: underline'>neither</span> is");
